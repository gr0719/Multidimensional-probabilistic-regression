...
public:
	...
	vector<H_node> time_windows;//当下纬度范围内的时间窗
	Lines_pmf F_C, F_A, F_T, F_N;//fc,fa
	Sketch S_C, S_A;//sc,sa
	vector<Data_object> Quarantine_area2;
	...
	void on_line_pmf(Data_object o_i, double t);
	void Materialize_node_from_m_to_o();
	void Materialize_node_from_m_to_o_Sketch();
private:
...
void H_node::Use_object_pmf_to_get_line_pmf_on_leaf_node()
{
	...
	*/
	...
	for (int i = 0; i < this->All_data_instance.size(); i++)
	{
		...
		//完整pmf
		if (pmf_i.isTruncated() == false){
			use_objects_to_get_pmfs(this->T, pmf_i, F_C);
			/*if (this->now_T < pmf_i.pmfs[0].x) {
				this->now_T = pmf_i.pmfs[0].x;
			}*/
		}
		{
			Quarantine_area.push_back(pmf_i);
...
void H_node::Materialize_node_from_m_to_o() {
	for (auto& child_node : this->Children_node_list)
	{
		if (child_node.isOnlineMazation == true) {
			break;
		}
		else {
			child_node.Materialize_node_from_m_to_o();
		}
	}
	//所有子结点都被物化了
	if (this->isOlayer == true) {
		return;
	}
	double start = clock();
...
void H_node::Materialize_node_from_m_to_o_Sketch() {
	for (auto& child_node : this->Children_node_list)
	{
		if (child_node.isOnlineMazation == true) {
			break;
			...
			child_node.Materialize_node_from_m_to_o_Sketch();
		}
	}
	//所有子结点都被物化了
	if (this->isOlayer == true) {
...
void H_node::on_line_pmf(Data_object o_i,double t)
{
	//On_linePmf(this->T, , Lines_pmf & F_x, double t)
	//On_linePmf(this->T, o_i, this->F_C, t);
	//RegressionOtimes(this->T, o_i, this->F_C, t);
	if (this->now_T >= t) {
		RegressionOtimes(this->now_T, o_i, this->F_C, t);
	}
	else {
		RegressionOdot(this->now_T, o_i, this->F_C, t);
		this->now_T = t;
	}

}
void H_node::on_line_sketch(Data_object o_i, double t)
{
	//On_linePmf(this->T, , Lines_pmf & F_x, double t)
	//On_linePmf(this->T, o_i, this->F_C, t);
	//RegressionOtimes(this->T, o_i, this->F_C, t);
	...
	if (this->now_T >= t) {
		RegressionOtimesSketch(this->now_T, o_i, this->S_C, t);
	}
	else {
		RegressionOdotSketch(this->now_T, o_i, this->S_C, t);
		this->now_T = t;
	}
